...
export default function Home() {
  ...
  return <main>
    ...
    <section className="panel key"><label>API KEY</label><input type="text" value={apiKey} onChange={(e) => { setApiKey(e.target.value); localStorage.setItem("streamApiKey", e.target.value); }} placeholder="회원가입 때 발급된 API 키"/><button onClick={() => void copyToClipboard(apiKey)} disabled={!apiKey}>복사</button><button onClick={load}>데이터 조회</button><button className="secondary" onClick={connectSocket} disabled={!apiKey}>Socket 연결</button></section>
